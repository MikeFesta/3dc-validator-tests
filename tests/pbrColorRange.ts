...
describe('pbr color check passing report', function () {
  ...
  before('load model', async function () {
    try {
      await v.model.loadFromFileSystem('models/blender-default-cube-pbr-safe-colors.glb');
    } catch (err) {
      throw new Error('Unable to load test model: blender-default-cube-pbr-safe-colors.glb');
    }
  });
  ...
  describe('max/min values not tested', function () {
    before('load schema', async function () {
      try {
        await v.schema.loadFromFileSystem('schemas/textures/pbr-color-range-no-check.json');
      } catch (err) {
  ...
  describe('max/min values within range', function () {
    before('load schema', async function () {
      try {
        await v.schema.loadFromFileSystem('schemas/textures/pbr-color-range-pass.json');
      } catch (err) {
...
describe('pbr color check failing report', function () {
  ...
  before('load model', async function () {
    try {
      await v.model.loadFromFileSystem('models/blender-default-cube-pbr-unsafe-colors.glb');
    } catch (err) {
  ...
  describe('max/min values outside of range', function () {
    before('load schema', async function () {
      try {
        await v.schema.loadFromFileSystem('schemas/textures/pbr-color-range-fail.json');
      } catch (err) {
